...
class CacheLine:
    """Linha de Cache (Mapeamento Direto)."""
    def __init__(self):
        self.valid = False
        self.tag = 0
        self.data = 0 
...
class MemorySystem:
    ...
    def read(self, address):
        ...
        index = real_addr % self.cache_size
        tag = real_addr // self.cache_size
...
class ALU:
    """ALU com Flags N (Negative) e Z (Zero)."""
    def __init__(self):
        self.n_flag = False 
        self.z_flag = False
        self.last_result = 0
    ...
    def compute(self, a, b, op):
        res = 0
        # Simula aritmética com sinal de 16 bits
...
class Mic1CPU:
    ...
    def __init__(self):
        ...
        # Inicializa SP no topo da memória
        self.sp.value = 4095

    def reset(self):
        """Reseta CPU mantendo o programa na memória."""
        registers = [self.mar, self.mdr, self.pc, self.mbr, self.lv, self.cpp, self.tos, self.opc, self.h]
        for r in registers: r.value = 0
        
        # Reset do SP para o topo da memória (Stack cresce para baixo)
        self.sp.value = 4095
        
        self.alu.n_flag = False
        self.alu.z_flag = False
        
        # Limpa cache, mas não a RAM
        self.memory.flush_cache()
        self.memory.last_access_status = "IDLE"
    ...
    def execute_instruction(self):
        ...
        # Lógica de Controle (Microcódigo Simulado)
        if opcode == 0x0: # LODD
            ...
            if not self.alu.n_flag and not self.alu.z_flag:
                ...
                self.control_signals = "JPOS (Not Taken)"
            ...
            if self.alu.z_flag:
                ...
                self.control_signals = "JZER (Not Taken)"
            self.bus_activity['bus_c'] = True
            ...
            eff_addr = (self.sp.value + addr_field) & 0xFFF # Usando SP como base para variáveis locais
            val = self.memory.read(eff_addr)
            self.h.value = self.alu.compute(val, 0, 'A')
            ...
            if self.alu.n_flag:
                ...
                self.control_signals = "JNEG (Not Taken)"
            self.bus_activity['bus_c'] = True
            ...
            if not self.alu.z_flag:
                ...
                self.control_signals = "JNZE (Not Taken)"
            self.bus_activity['bus_c'] = True
        ...
        elif opcode == 0xF: # Instruções Estendidas (Pilha e Halt)
            func = addr_field
            ...
            if func == 0: # HALT
            ...
            elif func == 1: # PSHI (Push Indireto)
                addr_ptr = self.h.value
                val = self.memory.read(addr_ptr)
                self.sp.value -= 1
            ...
            elif func == 2: # POPI (Pop Indireto)
                val = self.memory.read(self.sp.value)
                self.sp.value += 1
                addr_ptr = self.h.value
                ...
                ret_addr = self.memory.read(self.sp.value)
                self.sp.value += 1
